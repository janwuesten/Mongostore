...
async function action (store: Db, query: core.Query, auth: null, req: core.Request, res: core.Response): Promise<MongoStoreResponse> { 
    ...
    if(searchForId) {
        response = await mongostore.get(query.collection as string, query.document as string, null, store);
    }else{
        response = await mongostore.get(query.collection as string, JSON.parse(query.query as string), null, store);
    }
    return response;
};
